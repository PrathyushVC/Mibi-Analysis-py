...
def extract_patches(arr,patch_size=32,stride=32):
    if isinstance(patch_size,numbers.Number):
        patch_shape=tuple(patch_size*arr.ndim)
    if isinstance(stride,numbers.Number):
        extraction_step=tuple(stride*arr.ndim)
...
def main():
    ...
    test_set_size=0.10
    resize=1
    classes=np.arange(29)#6,7,13 we are actually only interested in these 3 classes and everything else can be labeled background for this test
    ...
    data_full_path = os.path.join(current_dir, r'..\..\..\Data_Full\**\*_CD4.npz')
    ...
    logging.info(f"Training set: {phases['train']}")
    logging.info(f"Validation set: {phases['val']}")
